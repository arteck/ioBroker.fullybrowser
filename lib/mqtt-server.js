            ...
            this.aedes.authenticate = (client, username, password, callback) => {
                try {
                    ...
                    if (ip && !Object.keys(this.adapter.fullysMQTT).includes(ip)) {
                        this.adapter.log.info(`[MQTT] Client ${client.id} not authorized: ${ip} is not an active Fully MQTT device IP per adapter settings.`);
                ...
                try {
                    ...
                    if (packet.retain) {
                        ...
                        if (!Object.keys(this.adapter.fullysMQTT).includes(ip)) {
                            this.adapter.log.error(`[MQTT] Packet rejected: IP ${ip} is not allowed per adapter settings. ${client.id}`);
                            return;
                        }
