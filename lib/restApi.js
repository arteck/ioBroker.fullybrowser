...
class RestApiFully {
    ...
    async sendCmd(device, cmd, val) {
        try {
            ...
            this.adapter.log.error(`[REST] ${device.id} IP ${device.name}: ${this.adapter.err2Str(e)}`);
            return false;
        }
    }
    ...
    async axiosSendCmd(device, cmd, urlParam, pic = false) {
        ...
        if (pic) {
            ...
            try {
                ...
                // Errors
                if (response.status !== 200) {
                    this.adapter.log.error(`[REST] ${device.id} IP ${device.name}: Sending command ${cmd} failed: ${response.status} - ${response.statusText}`);
                    return false;
                }

                if (response.data.status.includes('Error')) {
                    if (response.data.statustext == 'Please login') {
                        this.adapter.log.error(`[REST] ${device.id} IP ${device.name}: Error: Remote Admin Password seems to be incorrect. Sending command ${cmd} failed.`);
                    } else {
                        this.adapter.log.error(`[REST] ${device.id} IP ${device.name}: Error: Sending command ${cmd} failed, received status text: ${response.data.statustext}`);
                    }
                    return false;
                }
    ...
    errorFunction(err,device, cmd) {
        const errTxt = `[REST] ${device.id} IP ${device.name}: Sending command ${cmd} failed`;
